...
export async function loader({ params }: Route.LoaderArgs) {
    const response = await apiClient.surveys({ id: params.id as string }).results.get();
    if (!response.data) {
...
export default function Results() {
    ...
    const answers = results.answers.filter((answer) => answer.surveyId == results.id);

    return (
        <main className="p-4">
            <h1 className="text-2xl font-bold mb-4 text-center">Survey: {results.question}</h1>
